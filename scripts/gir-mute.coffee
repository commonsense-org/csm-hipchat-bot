...
module.exports = (robot) ->
  ...
  robot.hear /^(?=.*(gir))(?=.*?((be)? quiet|shut[ ]?up))/i, (msg) ->
    ...
    message = [
                ...
                "slinks away, head held low..."
                "vanishes in a puff of smoke... a cooked turkey is left behind"
              ]

    msg.emote msg.random message
    muteChannel action, channel, (what) ->
      msg.send what
...
muteChannel = (action, channel, cb) ->
  ...
  if action == 'unmute'
    cb 'ITS ME! I WAS THE TURKEY ALL ALONG!'
  return true
